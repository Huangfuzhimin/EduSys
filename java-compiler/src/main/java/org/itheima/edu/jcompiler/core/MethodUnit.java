...
public final class MethodUnit {

	protected String name;// 方法的名称
	...
	private Set<String> paramSignatures;
	...
	public Set<String> toSignatures() {
		...
		if (mix != null && mix.size() != 0) {
			...
			for (String str : mix) {
				set.add(str.concat(" ").concat(returnType).concat(" "));
			}

			prefixs = new ArrayList<>(set);
		} else {
			String prefix = returnType.concat(" ");
	...
	public Set<String> toParamSignatures() {
		if (paramSignatures != null) {
			return paramSignatures;
		}

		List<String> params = paramsCom();
		paramSignatures = new HashSet<>(params);
		return paramSignatures;
	}
